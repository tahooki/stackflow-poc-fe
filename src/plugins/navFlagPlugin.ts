...
const handleBeforePush: StackflowPluginPreEffectHook<PushActionParams> = ({
  ...
  if (!navFlag) {
    ...
    const activityParams = actionParams.activityParams as
      | UnknownRecord
      | undefined;
    if (
      activityParams &&
      Object.prototype.hasOwnProperty.call(activityParams, NAV_FLAG_FIELD)
    ) {
  ...
  switch (navFlag.flag) {
    ...
    case "CLEAR_STACK": {
      // 전체 스택을 비운 뒤 진입하되, 기존 스택이 있었다면 replace로 대체하여
      // 히스토리/초기 액티비티가 재삽입되며 2-depth가 되는 상황을 방지합니다.
